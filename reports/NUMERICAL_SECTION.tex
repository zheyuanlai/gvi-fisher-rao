...
No Monte Carlo enters the deterministic study: every expectation is computed by
deterministic quadrature, to the precision reported below. Gaussian targets are
closed form. The separable targets of
...
order $48$ and the objective evaluation and reference solve use order $96$, so the
evaluation rule is both distinct from and strictly finer than the update rule.
Measured against a rule of order $160$ on every dataset at the initial,
intermediate and reference states, the order-$48$ objective, gradient and Hessian
differ by at most $6.6\times10^{-11}$, $6.5\times10^{-12}$ and
$2.9\times10^{-11}$; order $96$ against order $192$ differs by at most
$1.3\times10^{-10}$ in the objective. Every gap the figures resolve is far above
these.
...
certifies the reference's suboptimality at least $6.9$ orders of magnitude below
the smallest gap any panel draws. We use the Fisher--Rao branch of the lemma,
$\Delta(a)\le\norm{\grad\mathcal E(a)}_a^2/(\alpha_\star\lambda_{\min}(C))$,
throughout; the Bures--Wasserstein form is slightly tighter but would put that
geometry inside the certification of a Fisher--Rao method rather than leaving it
an external baseline. The same inequality gives a reference-free certificate at
every iterate, which holds wherever the measured gap is meaningful and is tight to
a factor of $1.2$ on the log-cosh grid.
...
decimals on $13$ of the $15$ datasets and to within $1.9\times10^{-5}$ on all of
them, as they must, since they converge to the same optimizer; the differences between them are in how fast they
