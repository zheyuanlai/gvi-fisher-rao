...
\cite{diao2023forward} (\emph{FB-GVI}), which is used only as an external
baseline. On the logistic-regression problem the Laplace approximation is
...
Gaussian targets use closed-form expectations. The separable targets of
Section~\ref{subsec:numerics-nongaussian} use a panelled Gauss--Legendre rule
against the Gaussian density, which resolves the unit-width $\operatorname{sech}^2$
structure uniformly in the marginal width. The logistic posterior uses one
scrambled Sobol design of $4096$ points, shared between the updates, the
objective evaluation and the reference solve, so that every reported gap is exact
for the discretized problem the algorithms solve; the transfer error against an
independent design four times larger is reported separately and is of order
$10^{-3}$.

\paragraph{Reference solutions.}
Every non-Gaussian target is solved to a first-order-condition point whose
Fisher--Rao residual lies between $3\times10^{-13}$ and $9\times10^{-13}$. Since
the objective is quadratic at a stationary point, this places the reference's
objective error near $10^{-25}$, at least nine orders of magnitude below the
smallest gap any panel resolves.
...
are $2$ for both Fisher--Rao schemes and $1$ for FB-GVI, the latter being exactly
the largest stepsize admitted by \cite{diao2023forward}. Certified stepsizes are
...
runs about five per cent above it. The covariance bootstrap is therefore not
merely sufficient but sharp in its dependence on the initialization.
...
$\operatorname{cond}(S)$ ranging over eight decades, and each transformed
...
$\operatorname{cond}(S)=1$ through $10^{-12}$, $10^{-9}$ and $10^{-5}$. This is
...
congruence $C\mapsto SCS^{\!\top}$. At $\operatorname{cond}(S)=10^{8}$ the
transported covariance has condition number $\varepsilon_{\mathrm{mach}}^{-1}$
and is no longer representable in double precision, which is where the
measurement necessarily ends. FB-GVI, whose Bures--Wasserstein geometry is not
affine invariant, departs by an $O(1)$ amount as soon as $S$ is not orthogonal.
...
runs at its own certified stepsize. Both Fisher--Rao schemes reach the
double-precision floor within about $70$ iterations; FB-GVI reduces the KL gap by
less than a factor of two in $500$. The target is constructed so that coordinate
...
chosen so that $C_\star=\Id$ exactly, which is possible because the separable
optimizer does not depend on it. Starting from $m_0=2e_1$, $C_0=\Id$ then places
...
second-order term of the discretization: $10^{-5}$ on the targets with the
smaller certified step and $10^{-3}$ on those with the step five times larger.
...
$\kappa_X\in\{1,10^2,10^4\}$ and five independently generated datasets each. All
methods start from the prior, $m_0=0$, $C_0=\lambda^{-1}\Id$, which overshoots
$C_\star$ by about two orders of magnitude and so exercises the covariance
burn-in that the whitened targets of
Section~\ref{subsec:numerics-nongaussian} deliberately avoid.

All three iterative schemes reach the reference on every conditioning and every
dataset, with terminal Fisher--Rao gradient norms between $10^{-14}$ and
$10^{-8}$. Panels~(a) and~(b) of Figure~\ref{fig:logistic} show the objective gap
at $\kappa_X=10^4$ against iteration and against the time spent inside the update.

Two observations are worth recording, and they run in opposite directions.

First, no method dominates. The median iteration count to a $10^{-6}$ relative gap
is
\[
\begin{array}{lccc}
 & \kappa_X=1 & \kappa_X=10^2 & \kappa_X=10^4\\
\text{FR-R} & 1180 & 490 & 360\\
\text{FR-KL} & 1180 & 500 & 360\\
\text{FB-GVI} & 60 & 360 & 470.
\end{array}
\]
FB-GVI is an order of magnitude faster on the best-conditioned problem and slower
...
places as the conditioning grows. The two Fisher--Rao schemes are within two per
cent of one another throughout.

Second, the cost per iteration is the same for all three methods to within two
per cent, $246$ to $250$ milliseconds, so panel~(b) simply repeats the shape of
panel~(a). On a problem of this form the cost is dominated by the expectation,
which is $O(Snd)$ for $S=4096$ quadrature points and $n=500$ observations, and
not by the $O(d^3)$ linear algebra that separates the matrix exponential of
\eqref{upd:Riem} from the resolvent solve of \eqref{upd:KL}; the two differ by
about three orders of magnitude in flop count here. The distinction between the
two discretizations would become visible in wall-clock terms only at large $d$
...
by $0.008$, $0.005$ and $0.001$ respectively, while its objective gap is larger by
five to seven orders of magnitude. This is the familiar under-dispersion of the
reverse Kullback--Leibler objective, and it is a property of the variational
problem rather than of any algorithm that solves it.
