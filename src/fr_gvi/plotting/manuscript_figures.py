...
def _flow_time_panel(letter: str, global_sources: list[str]) -> Panel:
    return Panel(
            ...
            "iterations. On the eight cells with $\\kappa_{\\rm base}\\ge10$ the three "
            "schemes agree to within a factor of $2.2$, which places the iteration-count "
            "differences of panel (d) in the stepsize rather than in the geometry. The "
            "flow time does not grow with $\\kappa_\\star$ over the two decades the grid "
            "spans, so the worst-case constant is far from attained on this family. The "
            "four cells at $\\kappa_{\\rm base}=1$ are the exception, and for the reason "
            "given in panel (d): there $\\beta_\\star$ is smallest, the frozen multiplier "
            "overshoots hardest, and the Fisher--Rao flow time runs $7.6$ to $61$ times "
            "the Bures--Wasserstein one. That is the cost of freezing a single number "
...
def _scaling_cost_caption(data: pd.DataFrame) -> str:
    ...
    return (
        ...
        f"the algebra at every dimension measured, by a factor of {float(ratio.max()):.1f} "
        f"at most and {float(ratio.min()):.1f} at least, and it does not stop doing so: "
        ...
        "algebra is not what a method pays for, which is why the deterministic "
        "wall-clock panels of the main text largely repeat the shape of their "
        "iteration-count panels. The stochastic arms, whose oracle is subsampled, are "
        "the case where the balance reverses. Within the "
